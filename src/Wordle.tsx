  ...
  moves: [
	...
	useEffect(() => { 
		console.log('setting remaining turns');
			setRemainTurns([ ...Array.from({ length: gameData.remaining }, (): Attempt => {
				return [
	...
	<div className="main-card-container">
							...
							move.map(({letter, status}, letterIndex) => {
								...
								return (
										<div className={`card`} key={letterIndex}>
											<div className="inner">
												<div className={`front ${getStatusClassName(status)}`}>{letter}</div>
										</div>
									</div>
								);
							})
		...
		}
		{
			...
			remainingTurns?.map((remainingTurn, remainingTurnIndex) => {
				return (
					<div className={`card-container ${shouldFlip && remainingTurnIndex==0 ? 'card-container-flip':''}`} key={remainingTurnIndex}>
						{
							remainingTurn.map(({letter, status, willTransition}, letterIndex) => {

								return (
										<div className={`card placement-${letterIndex}`} key={letterIndex}>
											<div className={`inner ${!willTransition ? 'no-transition' : ''}`} style={
												{ 
  												...(willTransition && { transitionDelay: `${letterIndex * 150}ms` }) 
												}}
											>
												<div className={`front pending`}>{letter}</div>
												{willTransition && <div className={`back ${getStatusClassName(status)}`}>{letter}</div>}												
										</div>
									</div>
								);
							})
		...
		<button className="block" onClick={() => simulateSecondWordCheckResponse()}>
